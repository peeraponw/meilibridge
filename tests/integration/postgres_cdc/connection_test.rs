...
mod connection_tests {
    ...
    async fn test_connection_pool_behavior() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                ...
                let row = client.query_one(&format!("SELECT {} as num", i), &[]).await.unwrap();
                let num: i32 = row.get("num");
                assert_eq!(num, i);
            });
            handles.push(handle);
    ...
    async fn test_connection_with_invalid_credentials() {
        ...
        let invalid_connection = format!("postgresql://postgres:wrongpass@localhost:{}/testdb", port);
    ...
    async fn test_connection_parameter_validation() {
        ...
        let connection_strings = vec![
            ...
            format!("postgresql://postgres:postgres@localhost:{}/testdb?application_name=meilibridge", port),
        ];
    ...
    async fn test_reconnection_after_disconnect() {
        ...
        // Try to reconnect
        let (new_client, new_connection) = tokio_postgres::connect(&connection_string, tokio_postgres::NoTls)
            .await
            .unwrap();
    ...
    async fn test_connection_with_different_databases() {
        ...
        client.execute("CREATE DATABASE meilibridge_test", &[]).await.ok();
        
        // Connect to the new database
        let new_db = format!("postgresql://postgres:postgres@localhost:{}/meilibridge_test", port);
        ...
        let row = new_client.query_one("SELECT current_database()", &[]).await.unwrap();
