...
mod index_management_tests {
    ...
    async fn test_settings_synchronization() {
        ...
        // Create index
        let index = client.create_index("settings_test", Some("id"))
            .await
            .unwrap()
    ...
    async fn test_atomic_index_swap() {
        ...
        let primary_index = client.create_index(primary_name, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        primary_index.add_documents(&primary_docs, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        let staging_index = client.create_index(staging_name, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        staging_index.add_documents(&staging_docs, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        let doc = primary_after.get_document::<serde_json::Value>("1").await.unwrap();
    ...
    async fn test_schema_updates() {
        ...
        let index = client.create_index("schema_test", Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        index.add_documents(&extended_docs, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
    ...
    async fn test_multiple_index_management() {
        ...
        for name in &index_names {
            client.create_index(name, Some("id"))
                .await
                .unwrap()
                .wait_for_completion(&client, None, None)
                .await
    ...
    async fn test_index_stats_and_monitoring() {
        ...
        let index = client.create_index("stats_test", Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
    ...
    async fn test_index_recovery_after_error() {
        ...
        index.add_documents(&valid_docs, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        let wrong_key_docs = vec![
            json!({"wrong_id": 3, "title": "Wrong key"}),
        ];
        ...
        let more_docs = vec![
            json!({"id": 4, "title": "Valid 4"}),
        ];
        
        index.add_documents(&more_docs, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
