...
use meilibridge::models::event::{Event, EventType, EventId, EventSource, EventData, EventMetadata};
...
mod batch_processor_tests {
    ...
    fn create_test_event(id: i32, event_type: EventType) -> Event {
        ...
        data.insert("content".to_string(), json!(format!("Content for document {}", id)));
        ...
        Event {
            ...
            data: EventData {
                key: json!({"id": id}),
                old: if matches!(event_type, EventType::Delete) { Some(data.clone()) } else { None },
                new: if !matches!(event_type, EventType::Delete) { Some(data) } else { None },
            },
    ...
    async fn test_batch_size_optimization() {
        ...
        // Create index
        let index = client.create_index("batch_test", Some("id"))
            .await
            .unwrap()
        ...
        let task = index.add_documents(&small_batch, Some("id"))
            .await
            .unwrap();
        ...
        let task = index.add_documents(&large_batch, Some("id"))
            .await
            .unwrap();
    ...
    async fn test_partial_failure_handling() {
        ...
        let index = client.create_index("partial_failure_test", Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        let task = index.add_documents(&batch, Some("id"))
            .await
            .unwrap();
    ...
    async fn test_document_validation() {
        ...
        let index = client.create_index("validation_test", Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        let task = index.add_documents(&valid_docs, Some("id"))
            .await
            .unwrap();
    ...
    async fn test_performance_under_load() {
        ...
        let index = client.create_index("performance_test", Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        for batch_num in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                ...
                index_clone.add_documents(&batch, Some("id"))
                    .await
                    .unwrap()
    ...
    async fn test_update_operations() {
        ...
        let index = client.create_index("update_test", Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        index.add_documents(&initial_docs, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        index.add_documents(&updated_docs, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
    ...
    async fn test_delete_operations() {
        ...
        let index = client.create_index("delete_test", Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        index.add_documents(&docs, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
    ...
    async fn test_mixed_operations_batch() {
        ...
        let index = client.create_index("mixed_ops_test", Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        index.add_documents(&initial, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
        ...
        index.add_documents(&mixed, Some("id"))
            .await
            .unwrap()
            .wait_for_completion(&client, None, None)
            .await
