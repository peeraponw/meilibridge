...
mod redis_dlq_tests {
    ...
    fn create_dlq_entry(event: Event, error: MeiliBridgeError, retry_count: u32) -> serde_json::Value {
        json!({
            "event": event,
    ...
    async fn test_expiration_handling() {
        ...
        for i in 1..=5 {
            let event = create_test_event(i);
            let error = MeiliBridgeError::Io(std::io::Error::new(
        ...
        let remaining: Result<usize, redis::RedisError> = redis::cmd("ZCARD")
            .arg(dlq_zset)
            .query(&mut conn);
    ...
    async fn test_dlq_monitoring() {
        ...
        let success_rate = (metrics["reprocessed"] as f64 / metrics["total_failures"] as f64) * 100.0;
