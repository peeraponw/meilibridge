...
mod adaptive_batching_tests {
    ...
    async fn test_batch_size_adjustment_faster_than_target() {
        ...
        let initial_size = manager.get_batch_size(table_name, default_size, 10, 1000).await.unwrap();
        ...
        let new_size = manager.get_batch_size(table_name, default_size, 10, 1000).await.unwrap();
        assert!(new_size > initial_size, "Expected batch size to increase from {} to {}", initial_size, new_size);
    }
    ...
    async fn test_batch_size_adjustment_slower_than_target() {
        ...
        let new_size = manager.get_batch_size(table_name, default_size, 10, 1000).await.unwrap();
        assert!(new_size < default_size, "Expected batch size to decrease from {} to {}", default_size, new_size);
    }
    ...
    async fn test_multiple_table_isolation() {
        ...
        let size1 = manager.get_batch_size(table1, default_size, 10, 1000).await.unwrap();
        let size2 = manager.get_batch_size(table2, default_size, 10, 1000).await.unwrap();
    ...
    async fn test_metrics_window_sliding() {
        ...
        if let Some(table_stats) = stats.get(table_name) {
            assert!(table_stats.metrics_count <= 3, "Window should contain at most 3 metrics");
            assert!(table_stats.average_latency_ms > 200, "Average should reflect recent metrics");
        }
    }
    ...
    async fn test_minimum_metrics_required() {
        ...
        let size = manager.get_batch_size(table_name, default_size, 10, 1000).await.unwrap();
        assert_eq!(size, default_size, "Should not adjust with insufficient metrics");
    }
    ...
    async fn test_per_table_optimization_disabled() {
        ...
        let size = manager.get_batch_size(table_name, default_size, 10, 1000).await.unwrap();
        assert_eq!(size, default_size, "Should return default when optimization is disabled");
    }
    ...
    async fn test_concurrent_table_updates() {
        ...
        for (i, table) in tables.iter().enumerate() {
            ...
            let handle = tokio::spawn(async move {
                for j in 0..10 {
                    ...
                    manager_clone.record_metrics(&table_clone, metrics).await.unwrap();
        ...
        let stats = manager.get_statistics().await;
        for table in &tables {
            assert!(stats.contains_key(table), "Table {} should have statistics", table);
        }
    }
    ...
    async fn test_batch_size_boundaries() {
        ...
        let size = manager.get_batch_size(table_name, default_size, min_size, max_size).await.unwrap();
        assert!(size <= max_size, "Batch size {} should not exceed max {}", size, max_size);
        ...
        let final_size = manager.get_batch_size(table_name, default_size, min_size, max_size).await.unwrap();
        assert!(final_size >= min_size, "Batch size {} should not go below min {}", final_size, min_size);
    }
}