...
fn test_simple_field_mapping() {
    ...
    // Map "user_id" to "id"
    field_mappings.insert("user_id".to_string(), FieldMapping::Simple("id".to_string()));
    
    tables.insert("users".to_string(), TableMapping {
        name: None,
...
fn test_table_name_mapping() {
    let mut tables = HashMap::new();
    tables.insert("old_users".to_string(), TableMapping {
...
fn test_complex_mapping_with_default() {
    ...
    field_mappings.insert("status".to_string(), FieldMapping::Complex {
...
fn test_complex_mapping_composite_fields() {
    ...
    field_mappings.insert("first_name".to_string(), FieldMapping::Complex {
...
fn test_nested_field_extraction() {
    ...
    field_mappings.insert("address".to_string(), FieldMapping::Complex {
    ...
    let mut data = HashMap::new();
    data.insert("address".to_string(), json!({
        "street": "123 Main St",
        "city": "New York",
        "zip": "10001"
    }));
...
fn test_unmapped_fields_exclude_strategy() {
    ...
    // Only map id field
    field_mappings.insert("id".to_string(), FieldMapping::Simple("user_id".to_string()));
...
fn test_pass_through_other_events() {
    ...
    let event = Event::Checkpoint(Position::PostgreSQL { lsn: "0/1".to_string() });
...
fn test_value_to_string_conversion() {
    ...
    field_mappings.insert("num".to_string(), FieldMapping::Complex {
        to: Some("combined".to_string()),
        default: None,
        sources: Some(vec!["num".to_string(), "bool".to_string(), "str".to_string()]),
        separator: Some("-".to_string()),
        path: None,
    });
    
    tables.insert("test".to_string(), TableMapping {
        name: None,
        fields: Some(field_mappings),
    });
