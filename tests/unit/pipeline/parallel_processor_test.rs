use meilibridge::pipeline::parallel_processor::{ParallelConfig, ParallelTableProcessor, WorkStealingCoordinator};
...
async fn test_work_stealing_coordinator() {
    ...
    let processor1 = Arc::new(ParallelTableProcessor::new("table1".to_string(), config.clone()));
    let processor2 = Arc::new(ParallelTableProcessor::new("table2".to_string(), config.clone()));
    
    // Register processors
    coordinator.register_processor("table1".to_string(), processor1.clone()).await;
    coordinator.register_processor("table2".to_string(), processor2.clone()).await;
    ...
    assert!(processor2_queue_size > 0, "Work should have been stolen to processor2");
    
    // Check that processor1 has fewer events
    let processor1_queue_size = processor1.queue_size().await;
    assert!(processor1_queue_size < 200, "Some work should have been stolen from processor1");
