...
use meilibridge::recovery::{CircuitBreaker, CircuitState, CircuitBreakerConfig, CircuitBreakerManager};
use std::sync::Arc;
...
mod circuit_breaker_tests {
    ...
    async fn test_circuit_breaker_call_failure() {
        ...
        let result = breaker.call(simulate_success()).await;
        assert!(matches!(result, Err(meilibridge::recovery::circuit_breaker::CircuitBreakerError::CircuitOpen)));
    }
