...
mod postgres_types_tests {
    ...
    fn test_scalar_type_conversions() {
        ...
        // Test BIGINT
        assert_eq!(decode_value(b"9223372036854775807", oids::INT8).unwrap(), json!(9223372036854775807i64));
        assert_eq!(decode_value(b"-9223372036854775808", oids::INT8).unwrap(), json!(-9223372036854775808i64));
    ...
    fn test_string_type_conversions() {
        // Test VARCHAR/TEXT
        assert_eq!(decode_value(b"hello world", oids::TEXT).unwrap(), json!("hello world"));
        assert_eq!(decode_value(b"", oids::TEXT).unwrap(), json!(""));
        assert_eq!(decode_value(b"with \"quotes\"", oids::TEXT).unwrap(), json!("with \"quotes\""));
        assert_eq!(decode_value("unicode: 你好世界 🌍".as_bytes(), oids::TEXT).unwrap(), json!("unicode: 你好世界 🌍"));
        
        // Test CHAR
        assert_eq!(decode_value(b"A", oids::CHAR).unwrap(), json!("A"));
        assert_eq!(decode_value("🎉".as_bytes(), oids::CHAR).unwrap(), json!("🎉"));
    }

    #[test]
    fn test_array_type_conversions() {
        // Test INTEGER[]
        assert_eq!(decode_value(b"{1,2,3,4,5}", oids::INT4_ARRAY).unwrap(), json!([1, 2, 3, 4, 5]));

        // Test empty array
        assert_eq!(decode_value(b"{}", oids::INT4_ARRAY).unwrap(), json!([]));

        // Test STRING[]
        assert_eq!(decode_value(b"{hello,world}", oids::TEXT_ARRAY).unwrap(), json!(["hello", "world"]));
        assert_eq!(decode_value(b"{\"quoted value\",\"another\"}", oids::TEXT_ARRAY).unwrap(), json!(["quoted value", "another"]));

        // Test BOOLEAN[]
        assert_eq!(decode_value(b"{t,f,t}", oids::BOOL_ARRAY).unwrap(), json!([true, false, true]));

        // Test array with NULLs
        assert_eq!(decode_value(b"{1,NULL,3}", oids::INT4_ARRAY).unwrap(), json!([1, null, 3]));
    }
    ...
    fn test_date_time_conversions() {
        // Test DATE
        assert_eq!(decode_value(b"2024-01-15", oids::DATE).unwrap(), json!("2024-01-15"));

        // Test TIME
        assert_eq!(decode_value(b"14:30:45", oids::TIME).unwrap(), json!("14:30:45"));

        // Test TIMESTAMP
        assert_eq!(decode_value(b"2024-01-15 14:30:45", oids::TIMESTAMP).unwrap(), json!("2024-01-15 14:30:45"));

        // Test TIMESTAMPTZ
        assert_eq!(decode_value(b"2024-01-15 14:30:45+00", oids::TIMESTAMPTZ).unwrap(), json!("2024-01-15 14:30:45+00"));
        
        // Test INTERVAL
        assert_eq!(decode_value(b"1 year 2 months 3 days 04:05:06", oids::INTERVAL).unwrap(), json!("1 year 2 months 3 days 04:05:06"));
    }
    ...
    fn test_special_values() {
        ...
        // Test MONEY
        assert_eq!(decode_value(b"$1,234.56", oids::MONEY).unwrap(), json!(1234.56));
    ...
    fn test_bytea_conversion() {
        ...
        assert_eq!(decode_value(b"\\x00010203", oids::BYTEA).unwrap(), json!("\\x00010203"));
        assert_eq!(decode_value(b"\\x", oids::BYTEA).unwrap(), json!("\\x"));
        
        // Test regular text that doesn't start with \x
        assert_eq!(decode_value(b"some binary data", oids::BYTEA).unwrap(), json!("some binary data"));
    }

    #[test]
    fn test_network_types() {
        // Test INET
        assert_eq!(decode_value(b"192.168.1.1", oids::INET).unwrap(), json!("192.168.1.1"));
        assert_eq!(decode_value(b"2001:db8::1", oids::INET).unwrap(), json!("2001:db8::1"));
        assert_eq!(decode_value(b"192.168.1.0/24", oids::INET).unwrap(), json!("192.168.1.0/24"));
        
        // Test MACADDR
        assert_eq!(decode_value(b"08:00:2b:01:02:03", oids::MACADDR).unwrap(), json!("08:00:2b:01:02:03"));
    }
    ...
    fn test_edge_cases() {
        ...
        assert_eq!(decode_value(b"unknown type", 99999).unwrap(), json!("unknown type"));
    }

    #[test]
    fn test_array_edge_cases() {
        // Array with quoted commas
        assert_eq!(decode_value(b"{\"a,b\",\"c,d\"}", oids::TEXT_ARRAY).unwrap(), json!(["a,b", "c,d"]));
        
        // Array with empty string
        assert_eq!(decode_value(b"{\"\",\"nonempty\"}", oids::TEXT_ARRAY).unwrap(), json!(["", "nonempty"]));
        
        // Array with mixed NULL and values
        assert_eq!(decode_value(b"{NULL,1,NULL,2,NULL}", oids::INT4_ARRAY).unwrap(), json!([null, 1, null, 2, null]));
    }
}