...
impl TransactionalCheckpoint {
    ...
    pub async fn prepare(&self, transaction_id: &str) -> Result<bool> {
        ...
        match self.storage.save(&checkpoint).await {
            ...
            Err(e) => {
                error!("Failed to prepare checkpoint transaction {}: {}", transaction_id, e);
                Ok(false)
            }
        }
    }
    ...
    pub async fn commit(&self, transaction_id: &str) -> Result<()> {
        ...
        if !txn.prepared {
            return Err(MeiliBridgeError::Pipeline(
                format!("Transaction {} not prepared", transaction_id)
            ));
        }
        ...
        // Load the checkpoint
        let checkpoint = self.storage.load(&txn.task_id).await?
            .ok_or_else(|| MeiliBridgeError::Pipeline(
                format!("Checkpoint not found for task {}", txn.task_id)
            ))?;
