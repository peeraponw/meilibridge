...
impl TwoPhaseCommit {
    ...
    pub async fn prepare(
        ...
        if txn.state != TransactionState::Started {
            return Err(MeiliBridgeError::Pipeline(
                format!("Transaction {} in invalid state: {:?}", transaction_id, txn.state)
            ));
        }
    ...
    pub async fn commit(&self, transaction_id: &str) -> Result<()> {
        ...
        if txn.state != TransactionState::Prepared {
            return Err(MeiliBridgeError::Pipeline(
                format!("Transaction {} not prepared: {:?}", transaction_id, txn.state)
            ));
        }
    ...
    pub async fn cleanup_stale_transactions(&self) -> Result<()> {
        ...
        let stale_txns: Vec<String> = txns.iter()
            .filter(|(_, txn)| {
                now - txn.created_at > timeout && 
                txn.state != TransactionState::Committed
            })
            .map(|(id, _)| id.clone())
            .collect();
...
impl TransactionCoordinator {
    ...
    pub async fn begin(&self, task_id: &str) -> Result<String> {
        ...
        self.two_phase_commit.begin(transaction_id.clone(), task_id.to_string()).await?;
    ...
    pub async fn prepare(&self, transaction_id: &str, position: Position) -> Result<bool> {
        ...
        if let Some(handler) = checkpoint_handler.as_ref() {
            ...
            match timeout(Duration::from_secs(10), prepare_future).await {
                ...
                Ok(Ok(false)) => {
                    warn!("Checkpoint prepare failed for transaction: {}", transaction_id);
                    return Ok(false);
                }
                Ok(Err(e)) => {
        ...
        // Then prepare 2PC
        self.two_phase_commit.prepare(transaction_id, position).await?;
    ...
    pub async fn run_cleanup_task(&self) {
        ...
        loop {
            ...
            let checkpoint_handler = self.checkpoint_handler.read().await;
            if let Some(handler) = checkpoint_handler.as_ref() {
                if let Err(e) = handler.cleanup_stale_transactions(
                    self.two_phase_commit.timeout_secs
                ).await {
