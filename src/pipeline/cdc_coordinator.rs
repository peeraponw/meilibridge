...
impl CdcCoordinator {
    ...
    pub fn register_task(&mut self, table_name: String, tx: mpsc::Sender<Result<Event>>) {
        info!("Coordinator: Registering sync task for table '{}'", table_name);
        self.task_channels.insert(table_name.clone(), tx);
        debug!("Coordinator: Registered tasks: {:?}", self.task_channels.keys().collect::<Vec<_>>());
    }
    ...
    async fn distribute_event(&self, event: Event) {
        ...
        // Check if this specific table is paused
        {
            ...
            if paused_tables.contains(&table_name) || 
               (table_name.contains('.') && paused_tables.contains(table_name.split('.').last().unwrap_or(""))) {
        ...
        if table_name.contains('.') {
            ...
            if let Some(tx) = self.task_channels.get(table_only) {
                debug!("Sending event to task for table '{}' (without schema)", table_only);
    ...
    pub async fn pause_table(&self, table_name: &str) {
        self.paused_tables.write().await.insert(table_name.to_string());
        info!("CDC coordinator paused processing for table '{}'", table_name);
        metrics::CDC_CONTROL_OPERATIONS
    ...
    pub async fn resume_table(&self, table_name: &str) {
        ...
        info!("CDC coordinator resumed processing for table '{}'", table_name);
        metrics::CDC_CONTROL_OPERATIONS
    ...
    pub fn unregister_task(&mut self, table_name: &str) {
        info!("Coordinator: Unregistering sync task for table '{}'", table_name);
        self.task_channels.remove(table_name);
        debug!("Coordinator: Remaining tasks: {:?}", self.task_channels.keys().collect::<Vec<_>>());
    }
}