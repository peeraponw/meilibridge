...
impl RedisDlqStorage {
    pub fn new(redis_url: &str, key_prefix: String) -> Result<Self> {
        let client = Client::open(redis_url)
            .map_err(|e| MeiliBridgeError::Configuration(
...
impl DlqStorage for RedisDlqStorage {
    async fn store(&self, entry: DeadLetterEntry) -> Result<()> {
        ...
        let entry_data = serde_json::to_string(&entry)
            .map_err(|e| MeiliBridgeError::Serialization(e))?;
        ...
        redis::pipe()
            ...
            .await
            .map_err(|e| MeiliBridgeError::Redis(
                format!("Failed to store dead letter entry: {}", e)
            ))?;
    ...
    async fn get_by_task(&self, task_id: &str, limit: Option<usize>) -> Result<Vec<DeadLetterEntry>> {
        ...
        let entry_ids: Vec<String> = conn
            ...
            .await
            .map_err(|e| MeiliBridgeError::Redis(
                format!("Failed to get task entries: {}", e)
            ))?;
    ...
    async fn get_statistics(&self) -> Result<DlqStatistics> {
        ...
        let entry_ids: Vec<String> = conn
            ...
            .await
            .map_err(|e| MeiliBridgeError::Redis(
                format!("Failed to get all entries: {}", e)
            ))?;
    ...
    async fn clear_task(&self, task_id: &str) -> Result<usize> {
        ...
        if count > 0 {
            ...
            pipe.query_async::<_, ()>(&mut conn)
                .await
                .map_err(|e| MeiliBridgeError::Redis(
                    format!("Failed to clear task entries: {}", e)
                ))?;
...
impl RedisDlqStorage {
    async fn update_dlq_size_metric(&self, task_id: &str) -> Result<()> {
        ...
        let size: usize = conn
            ...
            .await
            .map_err(|e| MeiliBridgeError::Redis(
                format!("Failed to get task queue size: {}", e)
            ))?;
