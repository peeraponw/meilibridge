...
use crate::destination::circuit_breaker::{MeilisearchCircuitBreaker, CircuitBreakerBuilder, CircuitBreakerError};
...
impl ProtectedMeilisearchClient {
    ...
    pub async fn get_or_create_index(&self, index_name: &str, primary_key: Option<&str>) -> Result<Index> {
        if let Some(ref cb) = self.circuit_breaker {
            let client = self.client.clone();
                ...
                Box::pin(async move {
                    ...
                    match meilisearch.get_index(&index_name).await {
                        ...
                        Err(_) => {
                            ...
                            match meilisearch.create_index(&index_name, primary_key.as_deref()).await {
                                Ok(task) => {
                                    ...
                                    if task_info.is_success() {
                                        ...
                                        Err(MeiliBridgeError::Meilisearch(
                                            format!("Failed to create index: {:?}", task_info)
                                        ))
                                    }
                                }
            ...
            match meilisearch.get_index(index_name).await {
                Ok(index) => Ok(index),
                Err(_) => {
                    match meilisearch.create_index(index_name, primary_key).await {
                        Ok(task) => {
                            let task_info = meilisearch.wait_for_task(task, None, None).await
                                .map_err(convert_error)?;
    ...
    pub async fn add_documents(&self, index: &Index, documents: &[Value], primary_key: Option<&str>) -> Result<()> {
        if let Some(ref cb) = self.circuit_breaker {
            let client = self.client.clone();
            ...
            match cb.call(|| {
                Box::pin(async move {
                    match index.add_documents(&documents, primary_key.as_deref()).await {
                        Ok(task) => {
                            let task_info = client.client().wait_for_task(task, None, None).await
                                .map_err(convert_error)?;
                            ...
                            if task_info.is_success() {
                                ...
                                Err(MeiliBridgeError::Meilisearch(
                                    format!("Add documents failed: {:?}", task_info)
                                ))
                            }
                        }
                        Err(e) => Err(convert_error(e))
            ...
            match index.add_documents(documents, primary_key).await {
                Ok(task) => {
                    let task_info = self.client.client().wait_for_task(task, None, None).await
                        .map_err(convert_error)?;
                    ...
                    match index.delete_documents(&document_ids).await {
                        Ok(task) => {
                            ...
                            if task_info.is_success() {
                                ...
                                Err(MeiliBridgeError::Meilisearch(
                                    format!("Delete documents failed: {:?}", task_info)
                                ))
                            }
                        }
                        Err(e) => Err(convert_error(e))
            ...
            match cb.call(|| {
                Box::pin(async move {
                    match client.client().health().await {
                        Ok(health) => Ok::<bool, MeiliBridgeError>(health.status == "available"),
                        Err(_) => Ok(false),
                    }
                })
            }).await {
