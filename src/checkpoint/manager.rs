...
pub enum CheckpointCommand {
    Save { task_id: String, position: Position },
    Load { task_id: String, response: mpsc::Sender<Option<Checkpoint>> },
    Delete { task_id: String },
    List { response: mpsc::Sender<Vec<Checkpoint>> },
    Flush,
}
...
impl CheckpointManager {
    ...
    pub async fn save_checkpoint(&self, task_id: String, position: Position) -> Result<()> {
        if let Some(tx) = &self.command_tx {
            tx.send(CheckpointCommand::Save { task_id, position }).await
                .map_err(|_| MeiliBridgeError::ChannelSend)?;
            Ok(())
        } else {
            Err(MeiliBridgeError::Pipeline("Checkpoint manager not started".to_string()))
        }
    }
    ...
    pub async fn load_checkpoint(&self, task_id: &str) -> Result<Option<Checkpoint>> {
        if let Some(tx) = &self.command_tx {
            ...
            resp_rx.recv().await
                .ok_or(MeiliBridgeError::ChannelReceive)
        } else {
            Err(MeiliBridgeError::Pipeline("Checkpoint manager not started".to_string()))
        }
    }
    ...
    pub async fn list_checkpoints(&self) -> Result<Vec<Checkpoint>> {
        if let Some(tx) = &self.command_tx {
            let (resp_tx, mut resp_rx) = mpsc::channel(1);
            tx.send(CheckpointCommand::List { response: resp_tx }).await
                .map_err(|_| MeiliBridgeError::ChannelSend)?;
    ...
    async fn flush_pending(
        ...
        for checkpoint in checkpoints {
            match storage.save(&checkpoint).await {
                ...
                Err(e) => {
                    error!("Failed to save checkpoint for task '{}': {}", checkpoint.task_id, e);
        ...
        if !failed_tasks.is_empty() {
            warn!("{} checkpoints failed to save and will be retried", failed_tasks.len());
        }
    }
}
...
impl Position {
    ...
    pub fn is_after(&self, other: &Position) -> bool {
        match (self, other) {
            ...
            (Position::MySQL { file: f1, position: p1 }, Position::MySQL { file: f2, position: p2 }) => {
                f1 > f2 || (f1 == f2 && p1 > p2)
            }
